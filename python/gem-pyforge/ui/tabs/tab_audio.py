...
class TabAudio(ctk.CTkFrame):
    ...
    def _generate(self, prompt: str):
        ...
        request = EngineRequest(prompt=prompt, model_id="", modality="audio")
        task_queue.submit_task("audio_gen", self._run_engine, request)
    ...
    def _on_success(self, output, prompt):
        self.progress.grid_remove()
        self.viewer.show_text(f"Audio saved at: {output}")
        self.history.add_history(f"Audio: {prompt[:20]}...")
