...
class SpeechTab(ctk.CTkFrame):
    ...
    def _build_tab(self) -> None:
        ctk.CTkLabel(self, text="Speech settings", anchor="w").grid(
            row=0, column=0, sticky="w", padx=12, pady=(12, 8)
        )
        ctk.CTkLabel(
            self,
            text="Speech-to-text and text-to-speech controls for audio and transcription workflows.",
            wraplength=520,
            justify="left",
